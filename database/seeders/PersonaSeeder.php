...
// n_conCPI TOTAL DE PERSONAS CON CPI, TENDRÍAN QUE SER PERSONAS QUE NO TIENEN NINGÚN DOCUMENTO
        ...
        PersonaSeeder::set_n_personas(Persona::count());
        PersonaSeeder::set_n_noBinarios(Persona::where('id_sexo', 3)->count());
        PersonaSeeder::set_n_transGenero(Persona::where([
                                                            ['id_sexo', 1],
                                                            ['id_genero', '<>', 2]
                                                        ])->orWhere([
                                                            ['id_sexo', 2],
        ...
        PersonaSeeder::set_n_noArgentinos(Persona::where('nacionalidad_id_pais', '<>', 158)->count());
        PersonaSeeder::set_n_argNacidosExterior(Persona::where([
        ...
        if ($n_personas_menores == NULL) {
          PersonaSeeder::set_n_menores(0);
        } else {
          PersonaSeeder::set_n_menores($n_personas_menores);
        }
        // PersonaSeeder::set_n_menores(Persona::where('nacimiento_fecha'),'>','2005-01-01')->count();
        
        PersonaSeeder::set_n_fallecidos(Persona::where('vive_si', 0)->count());
     
        PersonaSeeder::set_n_sinDNI(Persona::where('id_documento_tipo','<>', 1)->count());
  
        PersonaSeeder::set_n_sinDNIsinDocExt(Persona::where([
                                                                ['id_documento_tipo','<>', 1],
                                                                ['posee_docExt_si', 0]
                                                            ])->count());
  
        PersonaSeeder::set_n_conCPI(Persona::where('posee_cpi_si','<>', 0)->count());
